  ...
  return function (_ref) {
    ...
    return {
      visitor: {
        ...
        Property: function Property(path, _ref4) {
          var opts = _ref4.opts;
          var node = path.node;

          buildDeclaratorHandler(node, 'value', path, opts);
        },
        VariableDeclarator: function VariableDeclarator(path, _ref5) {
          var opts = _ref5.opts;
          var node = path.node;

          buildDeclaratorHandler(node, 'init', path, opts);
        },
        LogicalExpression: function LogicalExpression(path, _ref6) {
          var opts = _ref6.opts;
          var node = path.node;

          buildExpressionHandler(node, ['left', 'right'], path, opts);
        },
        ConditionalExpression: function ConditionalExpression(path, _ref7) {
          var opts = _ref7.opts;
          var node = path.node;

          buildExpressionHandler(node, ['test', 'consequent', 'alternate'], path, opts);
        },
        IfStatement: function IfStatement(path, _ref8) {
          var opts = _ref8.opts;
          var node = path.node;
